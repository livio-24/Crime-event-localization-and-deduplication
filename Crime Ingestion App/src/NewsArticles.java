...
public class NewsArticles {
		...
		for (int count = 1; count <= pages; count++) {
			...
			for (Element element: links) {
				...
				if ((element.attr("href").startsWith("/cronaca/") || element.attr("href").startsWith("/video/")) && element.attr("href").endsWith(".html")) {
					link = "http://www.modenatoday.it" + element.attr("href");
					...
					try {
						...
						if (result == 1) {
							...
							if (address.equals(" ") || address.equals("")) {
								...
								pipeline.load();
								Annotation stanfordAnnotation = pipeline.runRaw(textBody);
							...
							Entity.extractEntities(link, concatenation_for_entities, conn);
	...
	Exception {
		...
		for (int count = 1; count <= pages; count++) {
			...
			for (Element element: links) {
				if (element.attr("href").startsWith("https://gazzettadimodena.gelocal.it/modena/cronaca/") || element.attr("href").startsWith("http://gazzettadimodena.gelocal.it/modena/cronaca/")
