...
public class NewsArticles {
	...
	public int addNewsMT(Connection conn, Integer pages, String url_news, String crime_type) throws FileNotFoundException, IOException, SQLException, ParseException, Exception {
		...
		for (int count = 1; count <= pages; count++) {
			...
			String link = "";
			org.jsoup.select.Elements links = document.select("a[href][class='o-link-text']");
				...
				if ((element.attr("href").startsWith("/cronaca/") || element.attr("href").startsWith("/video/")) && element.attr("href").endsWith(".html")) {
					...
					try {
						...
						if (result == 1) {
							...
							municipality = " ",
							address = " ";
							org.jsoup.select.Elements addresselement = news.select("span[class='u-label-02 u-inline-block u-no-underline u-mb-small']");
							...
							org.jsoup.select.Elements date = news.select("span[data-timestamp]");
							...
							Element cEntryDiv = body.select("div.c-entry").first();
							org.jsoup.select.Elements paragraphs = cEntryDiv.getElementsByTag("p");
							...
							for (Element paragraph: paragraphs) {
								...
								textBody = textBody.concat(paragraph.text() + " ");
							}
							...
							insertedNews++;
							System.out.println("X:" + insertedNews);
							// entities
